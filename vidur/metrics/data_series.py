...
class DataSeries:
    ...
    def plot_staircase(self, path: str, plot_name: str) -> None:
        # Staircase plot of the data series i.e y as a function of x
        if len(self._data_series) == 0:
            return
        ...
        if wandb.run:
            ...
            wandb.log(
                {
                    f"{plot_name}_staircase": wandb.plot.line(
                        wandb.Table(dataframe=wandb_df),
                        self._x_name,
                        self._y_name,
                        title=plot_name,
    ...
    def plot_rate(self, bucket_size: int, path: str, plot_name: str) -> None:
        if len(self._data_series) == 0:
            return

        df = self._to_df()
        df[self._x_name] = (df[self._x_name] // bucket_size) * bucket_size
        bucketed_df = df.groupby(self._x_name).agg({
            self._y_name: 'sum'
        }).reset_index()
        bucketed_df[self._y_name] = bucketed_df[self._y_name] / bucket_size

        self.print_series_stats(bucketed_df, plot_name)

        if wandb.run:
            wandb_df = bucketed_df.copy()
            wandb.log(
                {
                    f"{plot_name}_rate": wandb.plot.line(
                        wandb.Table(dataframe=wandb_df),
                        self._x_name,
                        self._y_name,
                        title=plot_name,
        ...
        self._save_df(bucketed_df, path, plot_name)