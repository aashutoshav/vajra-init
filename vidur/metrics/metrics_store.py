...
class MetricsStore:
    ...
    def __init__(self, simulation_config: SimulationConfig) -> None:
        ...
        for replica_idx in range(self._num_replicas):
            self._replica_memory_usage[replica_idx] = SeriesAverageMeter(
                TIME_STR,
                MEMORY_USAGE_STR,
                self._config.save_table_to_wandb,
            )
            ...
            self._replica_busy_time[replica_idx] = []
            self._replica_mfu[replica_idx] = []
        ...
        # autoscaling metrics
        self._autoscaling_metrics = {}

        self._autoscaling_metrics[AutoscalingMetrics.NUM_REPLICAS] = DataSeries(
            TIME_STR,
            AutoscalingMetrics.NUM_REPLICAS.value,
            self._config.subsamples,
            self._config.save_table_to_wandb,
            self._config.store_plots,
        )

        self._autoscaling_metrics[AutoscalingMetrics.COST_PER_HOUR] = SeriesAverageMeter(
            TIME_STR,
            AutoscalingMetrics.COST_PER_HOUR.value,
            save_table_to_wandb=self._config.save_table_to_wandb,
            x_scale=1 / 3600,
        )
    ...
    def _store_request_metrics(self, base_plot_path: str):
        ...
        if self._config.store_minimal_request_metrics:
            dataseries = self._request_metrics_time_distributions[RequestMetricsTimeDistributions.REQUEST_E2E_TIME]
    ...
    def _store_completion_metrics(self, base_plot_path: str):
        ...
        if self._config.store_minimal_request_metrics:
            dataseries = self._request_completion_metrics_time_series[RequestCompletionMetricsTimeSeries.REQUEST_ARRIVAL]
            dataseries.plot_step(
                base_plot_path, f"{dataseries._y_name}_time_series", COUNT_STR
            )
            dataseries.plot_rate(1, base_plot_path, f"{dataseries._y_name}_rate")
            return
    ...
    def _store_autoscaling_metrics(self, base_plot_path: str):
        if not self._config.store_autoscaling_metrics:
            return

        num_replicas_dataseries = self._autoscaling_metrics[AutoscalingMetrics.NUM_REPLICAS]
        num_replicas_dataseries.plot_staircase(base_plot_path, num_replicas_dataseries._y_name)

        self._autoscaling_metrics[AutoscalingMetrics.COST_PER_HOUR].print_stats(
            f"cost_per_hour", base_plot_path
        )
    ...
    def plot(self) -> None:
        ...
        self._store_autoscaling_metrics(dir_plot_path)
    ...
    @if_write_metrics
    def on_autoscaling_event(self, time: float, num_replicas: int, cost_per_hour: float) -> None:
        if not self._config.store_autoscaling_metrics:
            return

        self._autoscaling_metrics[AutoscalingMetrics.NUM_REPLICAS].put(time, num_replicas)
        self._autoscaling_metrics[AutoscalingMetrics.COST_PER_HOUR].put(time, cost_per_hour)

    @if_write_metrics
    def add_replica(self, replica_id: int) -> None:
        self._replica_memory_usage[replica_id] = SeriesAverageMeter(
            TIME_STR, MEMORY_USAGE_STR, self._config.save_table_to_wandb
        )
        self._replica_memory_usage[replica_id].put(0, 0)

        self._replica_busy_time[replica_id] = []
        self._replica_mfu[replica_id] = []

        for stage_idx in range(self._num_pipeline_stages):
            self._replica_busy_time[replica_id].append(
                SeriesAverageMeter(
                    TIME_STR,
                    BUSY_TIME_PERCENT,
                    save_table_to_wandb=self._config.save_table_to_wandb,
            ...
            self._replica_busy_time[replica_id][stage_idx].put(0, 0)

            self._replica_mfu[replica_id].append(
                SeriesAverageMeter(
                    TIME_STR,
                    UTILIZATION_STR,
                    save_table_to_wandb=self._config.save_table_to_wandb,
            ...
            self._replica_mfu[replica_id][stage_idx].put(0, 0)
